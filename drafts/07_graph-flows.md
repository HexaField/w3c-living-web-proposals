...
| ZCAPs [[GRAPH-GOVERNANCE]] | Who can act | "Only moderators can delete messages" |
...
5. Verifies role requirements via [[GRAPH-GOVERNANCE]] ([§9](#9-role-requirements)).
...
- **role** (OPTIONAL): A ZCAP action name ([[GRAPH-GOVERNANCE]] §4.5.3). If present, the firing agent MUST hold a valid ZCAP authorising this action on the context. See [§9](#9-role-requirements).
...
The "entered state at" timestamp is the reifier timestamp on the most recent `<instance> flow://state ?state` triple. This is sync-authoritative (per [[P2P-GRAPH-SYNC]] §22.9) and not subject to author manipulation.
...
A transition's `role` field names a ZCAP action ([[GRAPH-GOVERNANCE]] §4.5.3) that the firing agent MUST be authorised for. The flow engine MUST:

1. Resolve the role action.
2. Query [[GRAPH-GOVERNANCE]]'s `myCapabilities()` on the context (or directly check ZCAPs via `verify(agent, action, context, ...)`).
...
Registers a flow into this context. Requires an `updateFlow` capability ([[GRAPH-GOVERNANCE]] §4.5.3). Validates the JSON, writes the flow's triples to the context.
...
A transition is a write to the context — removing the old state link, adding the new state link, executing `actions`. These triples are gathered into a `ContextDiff` ([[P2P-GRAPH-SYNC]]) carrying the firing agent's `CapabilityProof`.
...
If two agents fire conflicting transitions concurrently, the sync protocol's conflict resolution applies. For OR-Set CRDT semantics (the default sync module in [[P2P-GRAPH-SYNC]]):
...
Flows are subject to the context's enforcement mode ([[GRAPH-GOVERNANCE]] §5):
...
An adversarial peer could spam transition attempts to win deterministic tie-breaks. The combination of ZCAP rate limits ([[GRAPH-GOVERNANCE]] caveats §9) and per-transition role requirements provides defence.
...
<dd><a href="./03_p2p-graph-sync.md">Peer-to-Peer Context Synchronisation</a>.</dd>
