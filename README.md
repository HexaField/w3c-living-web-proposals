...
- **ORM-style subject classes.** An ergonomic mapping layer that lets applications declare domain objects as decorated TypeScript/JavaScript classes — properties map to RDF predicates, collections to outbound link sets, flags to type assertions — with reads and writes flowing through the class instead of hand-written SPARQL. Conceptually an Object-Graph Mapper: define `class Message { @prop("msg://body") body: string; @collection("msg://reaction") reactions: Reaction[]; }`, then `await graph.find(Message).where({ author: alice })` instead of constructing query strings by hand. The substrate's content-addressed triples and SHACL shape semantics ([Spec 07](drafts/07_dynamic-graph-shape-validation.md)) already supply the typing and validation foundation; what's missing is the decorator vocabulary, the change-tracking strategy across distributed updates, and the schema-evolution story when a class definition is updated while peers still hold instances under the older shape.
