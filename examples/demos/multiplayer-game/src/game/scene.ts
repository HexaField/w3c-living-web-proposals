...
export function launchGame(container: HTMLElement, state: AppState): void {
    ...
    if (e.key === 'Enter' && !chatActive) {
      ...
      if (body) {
        ...
        if (v.allowed) {
          ...
          state.bc.postMessage({ type: 'game-chat', contextDid: state.graph.did, msg });
      ...
      if (dist < 1.2) {
        ...
        if (v.allowed) {
          ...
          state.bc.postMessage({
            type: 'game-collect', contextDid: state.graph.did,
  ...
  function broadcastPosition(): void {
    ...
    lastBroadcast = now;
    state.bc.postMessage({
      type: 'game-position', contextDid: state.graph.did,
