...
export function renderApp(root: HTMLElement, state: AppState): void {
  ...
  function render(): void {
    ...
    titleInput.addEventListener('change', () => {
      ...
      state.bc.postMessage({ type: 'doc-title-change', contextIri: state.graph.iri, did: state.did, title: titleInput.value });
    });
    ...
    shareBtn.addEventListener('click', () => {
      navigator.clipboard?.writeText(state.graph.iri);
    ...
    for (const block of state.blocks) {
      ...
      if (block.type !== 'divider') {
        ...
        // Comment on block via right-click graph
      ...
      if (!comment.resolved && state.myRole !== 'viewer') {
        ...
        replyBtn.addEventListener('click', () => {
          ...
          state.bc.postMessage({
            type: 'doc-new-reply', contextIri: state.graph.iri,
        ...
        if (state.myRole === 'owner' || state.myRole === 'editor') {
          ...
          resolveBtn.addEventListener('click', () => {
            ...
            state.bc.postMessage({
              type: 'doc-resolve-comment', contextIri: state.graph.iri, commentId: comment.id,
            });
            render();
          });
    ...
    if (commentBlockId && state.myRole !== 'viewer') {
      ...
      submitBtn.addEventListener('click', () => {
        ...
        state.bc.postMessage({
          type: 'doc-new-comment', contextIri: state.graph.iri,
