...
export function renderApp(container: HTMLElement, state: AppState): void {
  ...
  function renderLayers(): void {
    ...
    for (const layer of sorted) {
      ...
      vis.addEventListener('click', (e) => {
        ...
        state.bc.postMessage({ type: 'canvas-layer-toggle', contextIri: state.graph.did, layerId: layer.id, visible: layer.visible });
        renderLayers();
        renderShapes();
      });
      ...
      lock.addEventListener('click', (e) => {
        ...
        state.bc.postMessage({ type: 'canvas-layer-lock', contextIri: state.graph.did, layerId: layer.id, locked: layer.locked });
        renderLayers();
      });
    ...
    addBtn.addEventListener('click', () => {
      ...
      state.bc.postMessage({ type: 'canvas-layer-add', contextIri: state.graph.did, layer: newLayer });
      renderLayers();
    });
            ...
            if (state.currentTool === 'select') {
              ...
              const onUp = () => {
                ...
                state.bc.postMessage({
                  type: 'canvas-shape-move', contextIri: state.graph.did,
              ...
              state.bc.postMessage({
                type: 'canvas-shape-delete', contextIri: state.graph.did,
  ...
  svg.addEventListener('mousedown', (e: MouseEvent) => {
    ...
    if (tool === 'text') {
      ...
      state.bc.postMessage({ type: 'canvas-shape-add', contextIri: state.graph.did, shape });
  ...
  svg.addEventListener('mousemove', (e: MouseEvent) => {
    ...
    state.bc.postMessage({
      type: 'canvas-cursor', contextIri: state.graph.did,
    ...
    if (tool === 'rect') {
      ...
      state.bc.postMessage({
        type: 'canvas-stroke-progress', contextIri: state.graph.did,
