...
beforeEach(async () => {
  ...
  identity = eph;
  context = new Context('did:graph:test-shape-ctx', 'Test', identity, storage);
});
...
describe('Shape registration', () => {
  ...
  it('rejects re-registering the same shape name', async () => {
    await context.addShape('Task', JSON.stringify(TASK_SHAPE));
    await expect(context.addShape('Task', JSON.stringify(TASK_SHAPE))).rejects.toThrow(/already exists/);
  });

  it('removeShape removes the shape link from the context', async () => {
    await context.addShape('Task', JSON.stringify(TASK_SHAPE));
  ...
  it('getShapes returns registered shapes', async () => {
    await context.addShape('Task', JSON.stringify(TASK_SHAPE));
    const shapes = await context.getShapes();
...
describe('Shape instances', () => {
  ...
  it('createShapeInstance writes constructor triples', async () => {
    const uri = await context.createShapeInstance('Task', 'task:1', {
      title: 'Write spec',
      status: 'InProgress',
    });
  ...
  it('rejects when a required property is missing', async () => {
    await expect(
      context.createShapeInstance('Task', 'task:bad', { title: 'Without status' }),
    ).rejects.toThrow(/Required property "status"/);
  });

  it('getShapeInstances finds instances by type discriminator', async () => {
    await context.createShapeInstance('Task', 'task:1', { title: 'A', status: 'X' });
    await context.createShapeInstance('Task', 'task:2', { title: 'B', status: 'Y' });
    const instances = await context.getShapeInstances('Task');
    expect(instances.sort()).toEqual(['task:1', 'task:2']);
  });

  it('getShapeInstanceData returns scalar + collection properties', async () => {
    await context.createShapeInstance('Task', 'task:1', { title: 'A', status: 'X' });
    await context.addToShapeCollection('Task', 'task:1', 'assignees', 'did:key:z6MkAlice');
    await context.addToShapeCollection('Task', 'task:1', 'assignees', 'did:key:z6MkBob');
    const data = await context.getShapeInstanceData('Task', 'task:1');
  ...
  it('setShapeProperty replaces a scalar value', async () => {
    await context.createShapeInstance('Task', 'task:1', { title: 'A', status: 'X' });
    await context.setShapeProperty('Task', 'task:1', 'status', 'Done');
    const data = await context.getShapeInstanceData('Task', 'task:1');
  ...
  it('addToShapeCollection rejects when called on a scalar property', async () => {
    await context.createShapeInstance('Task', 'task:1', { title: 'A', status: 'X' });
    await expect(
      context.addToShapeCollection('Task', 'task:1', 'title', 'B'),
  ...
  it('removeFromShapeCollection removes the value', async () => {
    ...
    await context.addToShapeCollection('Task', 'task:1', 'assignees', 'did:key:z6MkAlice');
    await context.addToShapeCollection('Task', 'task:1', 'assignees', 'did:key:z6MkBob');
    await context.removeFromShapeCollection('Task', 'task:1', 'assignees', 'did:key:z6MkBob');
    const data = await context.getShapeInstanceData('Task', 'task:1');
...
describe('Shape validation', () => {
  it('rejects malformed JSON', async () => {
    await expect(context.addShape('Bad', '{ invalid }')).rejects.toThrow(/Invalid JSON/);
  });

  it('rejects shape without targetClass', async () => {
    await expect(context.addShape('Bad', JSON.stringify({ properties: [], constructor: [] }))).rejects.toThrow(/targetClass/);
  });
