...
| [`@living-web/capability-framework`](./polyfills/capability-framework/) | [04 — Graph Capability Framework](../drafts/04_graph-capability-framework.md) | Scope-set governance (hierarchical + holonic); accumulate + deny-wins; BootstrapRoot chain-cut; immutable-caveats attenuation; Open/Announced/Enforced; plug-in constraint-kind registry |
| [`@living-web/context-sync`](./polyfills/context-sync/) | [05 — Context Sync Protocol](../drafts/05_context-sync-protocol.md) | GraphDiff (graph-DID-keyed) + sync spaces + per-graph subscription; sync-blocking respects the full scope-set governance |
